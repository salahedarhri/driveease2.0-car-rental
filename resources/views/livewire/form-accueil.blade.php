        ...
        <div class="grid lg:grid-cols-2 grid-cols-1 justify-center max-lg:gap-2">
            ...
            <div class="flex flex-row max-sm:flex-col w-full max-lg:gap-2">
                ...
                <label for="lieuDepart" class="w-full" x-data="{ lieuDepart:''}">
                    ...
                    <input name="lieuDepart" type="text" placeholder="Lieu de départ"
                    ...
                    <div wire:transition class="absolute z-30 mt-1 w-80 bg-white rounded shadow-xl p-2 font-cabin">
                        ...
                        <div wire:loading.remove wire:target="lieuDepart">
                            ...
                            <p wire:click="appliquerLieuDepart('{{ $lieu->nom }}','{{ $lieu->id }}')"
                                x-on:click="lieuDepart='{{ $lieu->nom }}'"
                                class="truncate px-2 py-2 cursor-pointer hover:bg-teal-100">
                                <i class="font-normal ri-bus-fill text-xl text-teal-500 mr-3"></i>{{ $lieu->nom }}
                            </p>
                            @endforeach
                        </div>
                        @endif
                ...
                <label for="lieuRetour" class="w-full" x-data="{ lieuRetour:''}">
                    ...
                    <input name="lieuRetour" type="text" placeholder="Lieu de retour"
                        wire:model.live.debounce.100ms="lieuRetour" autocomplete="off" x-model="lieuRetour"
                    ...
                    <div wire:transition class="absolute z-30 w-80 mt-1 bg-white rounded shadow-md p-2 font-cabin">
                        ...
                        <p wire:loading.remove wire:target="lieuRetour" class=" text-teal-600 underline text-sm">
                            Aéroports :</p>
                        <div wire:loading.remove wire:target="lieuRetour">
                            @foreach ($lieuxRetour->where('type','Aéroport') as $lieu)
                            <p wire:loading.remove
                                wire:click="appliquerLieuRetour('{{ $lieu->nom }}','{{ $lieu->id }}')"
                                x-on:click="lieuRetour='{{ $lieu->nom }}'"
        ...
        <div class="flex flex-row align-middle justify-between pt-2 text-sm mx-2">
            <div class="flex flex-row gap-2 place-items-center">
                ...
                <select name="minAge" wire:model.blur="minAge" class="border-none focus:border-none">
                    @for($i = 18; $i <= 25; $i++) <option value="{{ $i }}">{{ $i }}</option>
                        @endfor
            ...
            <button type="submit"
                class="px-5 py-2 bg-teal-500 hover:bg-teal-600 text-white cursor-pointer font-semibold rounded-lg shadow-md uppercase transition-all">
                Rechercher
            </button>
        </div>
    </form>
