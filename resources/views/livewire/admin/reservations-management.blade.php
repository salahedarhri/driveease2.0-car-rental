<div>
    ...
    <p class="text-lg max-md:text-md text-mediumBlue p-3 mt-3 font-montserrat font-semibold">Liste des réservations</p>
    ...
    <div class="flex flex-row max-sm:flex-col max-sm:text-center max-sm:gap-3 justify-between place-items-center p-3 mt-2 font-cabin">
      ...
      <button class="text-white bg-mediumBlue hover:saturate-150 transition rounded py-1 px-3 flex flex-row place-items-center gap-2">
        <i class="ri-add-circle-line text-2xl"></i>
        <p class="font-montserrat font-semibold">Ajouter une réservation</p>
      </button>
    </div>
            ...
            <tr class="border-b-slate-300 hover:bg-sky-100">
              <td>
                <div class="w-full h-full avatar flex items-center justify-center" @click="openDisplay=true; imageChoisi='{{ asset('images/voitures/'.$voiture->photo ) }}' ">
                  <div class="mask h-8 w-14"><img class="object-contain" src="{{ asset('images/voitures/'.$voiture->photo ) }}" alt="car image"/></div></div>
              </td>
